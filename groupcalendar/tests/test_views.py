...
class GroupCalendarViewTests(TestCase):
    ...
    def test_calendar_detail(self):
        ...
        response = self.client.get(reverse('calendar_detail',
    ...
    def test_calendar_update(self):
        calendar = self._generate_default_calendar()
        response = self.client.get(reverse('calendar_update',
            args={calendar.id}), follow=True)
        self.assertContains(response, 'form')
        # TODO: continue
    ...
    def test_calendar_delete(self):
        ...
        response = self.client.get(reverse('calendar_delete',
            args={calendar.id}), follow=True)
        post_response = self.client.post(reverse('calendar_delete',
            args=(calendar.id,)), follow=True)
        self.assertRedirects(post_response, reverse('calendar_list'), status_code=302)
